...
class RNNModel(object):
    ...
    builder rnn model
    """
    def __init__(self,is_test,hidden_size,rnn_layers,batch_size,seq_length,
                 vocab_size,embedding_size,learning_rate, max_grad_norm):
        ...
        self.initial_state=create_tuple_placeholder_with_default(
            self.zero_state,shape=multi_cell.state_size)
        ...
        self.embeding=tf.get_variable('embedding',[vocab_size,embedding_size])
        inputs=tf.nn.embedding_lookup(self.embeding,self.input_data)
        sliced_inputs=[tf.squeeze(input_,[1]) for input_ in tf.split(axis=1,num_or_size_splits=self.seq_length,value=inputs)]
        ...
        with tf.control_dependencies([update_loss_monitor]):#依赖update_loss_monitor操作,即完成update_loss_monitor操作后继续往后执行
            #保证按顺序执行的正确性
...
def create_tuple_placeholder_with_default(inputs,shape):
    if isinstance(shape,int):
        result=tf.placeholder_with_default(input,list((None,))+[shape])
    else:
        subplaceholders=[create_tuple_placeholder_with_default(subinputs,subshape)
