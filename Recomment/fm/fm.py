...
class FM(object):
    ...
    def add_inference(self):
        ...
        with tf.variable_scope('interaction_layer'):
            ...
            # interaction_layer
            self.interaction_terms=tf.multiply(0.5,tf.reduce_mean(tf.subtract(tf.pow(
                tf.sparse_tensor_dense_matmul(self.X,v),2),
                tf.sparse_tensor_dense_matmul(tf.pow(self.X,2),tf.pow(v,2))),axis=1,keep_dims=True
            ))
            #shape of[None,2]
    ...
    def add_loss(self):
        cross_entroy=tf.nn.sparse_softmax_cross_entropy_with_logits(self.y,self.y_out_prob)
        mean_loss=tf.reduce_mean(cross_entroy)
        self.loss = mean_loss
        tf.summary.scalar('loss', self.loss)
    def add_accuracy(self):
        #accuracy
        self.correct_prediction=tf.equal(tf.cast(tf.argmax(self.y_out,1),tf.int64),self.y)
        self.accuracy=tf.reduce_mean(tf.cast(self.correct_prediction,tf.int32))/len(self.correct_prediction)
    ...
    def train(self):
        ...
        extra_update_ops= tf.get_collection(tf.GraphKeys.UPDATE_OPS)#网络更新操作
        with tf.control_dependencies(extra_update_ops):#等待更新完成，再更新参数
...
def train_model(sess,model,epoches=10,print_every=0):
    """
    train model
    :param sess:
    :param model:
...
def test_model(sess, model, print_every = 50):
    ...
    with open('./../data/test_sparse_data_frac_0.01.pkl', 'rb') as f:
