...
def train_model(sess,model,epochs=10,print_every=500):
    ...
    for e in range(epochs):
        ...
        train_data = pd.read_csv('E:\\documents\\ml-dataset\\avazu-ctr-predictiond\\train.csv',
...
def test_model(sess,model,print_every=50):
    ...
    test_data = pd.read_csv('E:\\documents\\ml-dataset\\avazu-ctr-predictiond\\test.csv',
                            chunksize=model.batch_size)
    test_step=1
    # loading dicts
    fields_dict = {}
    ...
    for data in test_data:
        ...
        for i in range(actual_batch_size):
            sample = data.iloc[i,:]
            array=one_hot_representation(sample,fields,test_array_length)
